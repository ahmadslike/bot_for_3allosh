...
# ======================
# ⏱️ إعدادات لوحة TimeSMS
# ======================
TIMESMS_DASHBOARD = {
    "name": "TimeSMS",
    "type": "timesms",
    "base_url": "http://www.timesms.org",
    "login_url": "http://www.timesms.org/login",
    "username": "HAMOELNOS7",
    "password": "6634032038",
    "session": requests.Session(),
    "is_logged_in": False,
    "last_check": None,
    "sent_messages_file": "sent_messages_timesms.json"
}
...
def send_welcome(message):
    ...
    fancy_text = (
        "<b>𝐖𝐞𝐥𝐜𝐨𝐦 𝐭𝐨 Ahmad & Hamo 𝑶𝑻𝑷</b>\n\n"
        "<b>👨🏻‍💻 𝑶𝑾𝑬𝑵𝑹💫  •  <a href='https://t.me/A7mad_1m'>𝑶𝑾𝑬𝑵𝑹✨</a>  •  <a href='https://t.me/MO7_TF7'>𝑶𝑾𝑬𝑵𝑹✨</a></b>\n\n"
        "<b>────────────────────</b>\n"
        "<b>𝐒𝐞𝐥𝐞𝐜𝐭 𝐂𝐨𝐮𝐧𝐭𝐫𝐲 : ⬇️</b>"
    )
...
# --- دوال TimeSMS ---
def login_to_timesms():
    """تسجيل الدخول إلى TimeSMS"""
    dash = TIMESMS_DASHBOARD
    try:
        session = dash["session"]
        session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
        })
        login_page = session.get(f"{dash['base_url']}/login", timeout=30)
        soup = BeautifulSoup(login_page.text, 'html.parser')
        token = soup.find('input', {'name': '_token'})
        csrf = token['value'] if token else None

        data = {'username': dash['username'], 'password': dash['password']}
        if csrf:
            data['_token'] = csrf

        resp = session.post(f"{dash['base_url']}/login", data=data, timeout=30, allow_redirects=True)
        if 'login' not in resp.url.lower():
            print(f"[TimeSMS] ✅ تسجيل الدخول ناجح")
            dash['is_logged_in'] = True
            return True
        else:
            print(f"[TimeSMS] ❌ فشل تسجيل الدخول")
            return False
    except Exception as e:
        print(f"[TimeSMS] ❌ خطأ: {e}")
        return False

def fetch_timesms_messages():
    """جلب رسائل SMS من TimeSMS"""
    dash = TIMESMS_DASHBOARD
    if not dash.get('is_logged_in'):
        if not login_to_timesms():
            return []
    try:
        session = dash["session"]
        resp = session.get(f"{dash['base_url']}/sms/received", timeout=30)
        soup = BeautifulSoup(resp.text, 'html.parser')
        messages = []
        ...
        for row in rows:
            ...
            if len(cells) >= 3:
                ...
                text = cells[1].get_text().strip() if cells[1] else ''
                if number and text and len(text) > 3:
                    msg_id = f"timesms-{number}-{text[:30]}"
                    messages.append({
                        'id': msg_id,
                        'number': number,
                        'text': text,
        ...
        print(f"[TimeSMS] 📨 وجد {len(messages)} رسالة")
        return messages
    except Exception as e:
        print(f"[TimeSMS] ❌ خطأ في جلب الرسائل: {e}")
...
def main_loop():
    ...
    # قائمة باللوحتين
    DASHBOARDS = [IVASMS_DASHBOARD, TIMESMS_DASHBOARD]
    ...
    while True:
        for dash in DASHBOARDS:
            try:
                ...
                # جلب الرسائل حسب نوع اللوحة
                if dash.get('type') == 'timesms':
                    messages = fetch_timesms_messages()
                else:
