...
def handle_country_selection(call):
    ...
    # زر الجروب في الأعلى
    markup.add(types.InlineKeyboardButton("OTP GROUP", url="https://t.me/otp3llosh2"))
...
def login_to_ivasms():
    try:
        ...
        # حقن الكوكيز مباشرة لتجاوز Cloudflare
        IVASMS_COOKIES = {
            'ivas_sms_session': '%3D',
            'XSRF-TOKEN': '%3D',
        }
        ...
        if "login" not in test.url.lower():
            ...
            print(f"[{dash['name']}] ❌ فشل تسجيل الدخول، الكوكيز منتهية")
            time.sleep(300)
            return False
    except Exception as e:
        ...
        for group_id in group_ids:
            ...
            for phone in phone_numbers:
                try:
                    ...
                    # استخراج الرسائل من جدول iVasms
                    # الجدول: Sender | Message | Time | Revenue
                    extracted = []

                    # طريقة 1: من tbody أو tr مباشرة
                    for row in soup3.find_all('tr'):
                        cells = row.find_all('td')
                        if len(cells) >= 2:
                            # العمود الثاني هو Message
                            msg_cell = cells[1].get_text(separator=' ').strip()
                            # تجاهل الصفوف اللي ما فيها نص SMS حقيقي
                            if msg_cell and len(msg_cell) > 5 and any(c.isdigit() for c in msg_cell):
                                extracted.append(msg_cell)

                    # طريقة 2: ابحث عن أي td يحتوي على كود أو رسالة SMS
                    if not extracted:
                        for td in soup3.find_all('td'):
                            text = td.get_text(separator=' ').strip()
                            if len(text) > 10 and any(c.isdigit() for c in text) and ('code' in text.lower() or 'otp' in text.lower() or 'verify' in text.lower() or '-' in text):
                                extracted.append(text)

                    # طريقة 3: أي نص طويل في الصفحة
                    if not extracted:
                        full = soup3.get_text(separator=' ')
    ...
    if user_id:
        try:
            ...
            markup.row(
                ...
                types.InlineKeyboardButton("💬𝐎𝐓𝐏", url="https://t.me/otp3llosh2")
            )
            bot.send_message(
                user_id,
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [
                    {"text": "💬 𝑺𝑴𝑺 ↗️", "url": "https://t.me/otp3llosh2", "style": "primary"},
                    {"text": "🤖 𝑩𝑶𝑻 ↗️", "url": "https://t.me/Y_I_l_l56BOT", "style": "success"}
                ],
                
                # الصف الثالث: زر المطور
                [
                    {"text": "👤 M:𝑩𝑶𝑻 ↗️", "url": "https://t.me/Y_I_l_l56BOT", "style": "danger"}
                ]
            ]
        }
    except Exception as e:
